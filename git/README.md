...
- 2.把仓储门口的代码放到里面的房间中去
    ...
    + (没有-m 会进入另外一个界面。按'i'-可以进行编辑；按'esc'+':q'退出；按'esc'+':q!'强制退出)
    + ![](images/git仓储.png)
...
- 命令: `git status`
   + modified绿色 表示文件已经放在了大门口
   + modified红色 表示文件已修改，还未放在大门口
## git中的忽略文件
...
## 查看日志
- `git log` 查看历史提交的日志（很多可以按q在进入命令行）
...
## 在push和pull操作进
- 先pull , 再push
- git remote add origin "git@github.com:JIAjia-hjj/text119.git"
